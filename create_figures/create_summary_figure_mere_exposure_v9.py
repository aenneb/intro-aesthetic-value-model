...
def extend_fit_results(fit_results_raw):
    ...
    fit_results['low_rmse'] = fit_results.rmse<np.quantile(fit_results.rmse,0.1)
...
#%% get fit results
fit_results_raw_inv_u = pd.read_csv(home_dir + '/results'
                          + '/fit_results_Montoya_2017.csv', index_col=False)
fit_results_raw_u = pd.read_csv(home_dir + '/results'
                          + '/fit_results_Montoya_2017_u_shape.csv', index_col=False)
fit_results_raw_increase = pd.read_csv(home_dir + '/results'
                          + '/fit_results_Montoya_2017_exponential.csv', index_col=False)
fit_results_raw_decrease = pd.read_csv(home_dir + '/results'
                          + '/fit_results_Montoya_2017_decrease.csv', index_col=False)

fit_results_inv_u, best_fit_inv_u = extend_fit_results(fit_results_raw_inv_u)
fit_results_u, best_fit_u = extend_fit_results(fit_results_raw_u)
fit_results_increase, best_fit_increase = extend_fit_results(fit_results_raw_increase)
fit_results_decrease, best_fit_decrease = extend_fit_results(fit_results_raw_decrease)
...
data_inv_u = np.polyval([meta_quadratic, meta_slope, meta_intercept], reps)
predictions_inv_u = best_fit_inv_u.iloc[:,10:60].values.flatten()
# u shaped
meta_intercept = .69631
meta_slope = - .00360
meta_quadratic = .0000810
data_u = np.polyval([meta_quadratic, meta_slope, meta_intercept], reps)
predictions_u = best_fit_u.iloc[:,10:60].values.flatten()
#increase
data_increase = exponential(1,.1,reps)
predictions_increase = best_fit_increase.iloc[:,10:60].values.flatten()
#decrease
meta_intercept = .54850
meta_slope = -.0017
meta_quadratic = .000016
data_decrease = np.polyval([meta_quadratic, meta_slope, meta_intercept], reps)
predictions_decrease = best_fit_decrease.iloc[:,10:60].values.flatten()
...
X, Y, Z_X, Z_true = get_dist_plot_data(best_fit_inv_u,
                                       xmin=-10, ymin = -10, xmax=60, ymax=60)
plot_mere_exposure(data_inv_u, predictions_inv_u, best_fit_inv_u,
                   X, Y, Z_X, Z_true, ax0, ax1, 'A', legends=True)

X, Y, Z_X, Z_true = get_dist_plot_data(best_fit_u,
                                        xmin=-10, ymin = -10, xmax=10, ymax=10)
plot_mere_exposure(data_u, predictions_u, best_fit_u, X, Y, Z_X, Z_true,
                    ax2, ax3, 'B')

X, Y, Z_X, Z_true = get_dist_plot_data(best_fit_increase,
                                        xmin=-30, ymin = -30, xmax=10, ymax=10)
plot_mere_exposure(data_increase, predictions_increase, best_fit_increase,
                    X, Y, Z_X, Z_true, ax4, ax5, 'C')

X, Y, Z_X, Z_true = get_dist_plot_data(best_fit_decrease,
                                        xmin=-10, ymin = -10, xmax=10, ymax=10)
plot_mere_exposure(data_decrease, predictions_decrease, best_fit_decrease,
                    X, Y, Z_X, Z_true, ax6, ax7, 'D')

fig.tight_layout(pad=.5)
# plt.savefig('summary_fig_mere_exposure_v8.png')
